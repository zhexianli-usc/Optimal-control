"""
Train FNO for 1D Burgers equation using discrete Fourier transform with real frequency only.
Maps boundary condition (g_left, g_right) -> solution u(x,t). Same structure as train_fno_heat_bc_real_freq.py.
Data: burgers_bc_data/burgers_bc_data.npz.
"""
import csv
import os
import numpy as np
...
def bc_enc_to_field(bc_enc, n_x_pts, n_t_pts):
    ...
    field = np.zeros((n_samples, 2, n_x_pts, n_t_pts), dtype=np.float32)
    field[:, 0, :, :] = g_left[:, None, :]
...
def main():
    ...
    model_path = os.path.join(OUT_DIR, "fno_burgers_bc_real_freq_model.pt")
    torch.save(checkpoint, model_path)
    print(f"  Model saved to {model_path}")

    loss_csv = os.path.join(OUT_DIR, "train_fno_burgers_bc_real_freq_loss_history.csv")
    ...
    np.savetxt(
        os.path.join(OUT_DIR, "train_fno_burgers_bc_real_freq_mse_orig_hist.csv"),
    ...
    ax1.set_title("FNO Burgers (real frequency) training MSE")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, "train_fno_burgers_bc_real_freq_mse.png"), dpi=150)
    plt.close()
...
if __name__ == "__main__":
    print("FNO Burgers (real frequency only)")

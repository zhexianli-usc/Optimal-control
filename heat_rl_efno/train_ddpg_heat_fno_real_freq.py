"""
Actor–critic RL for additive heat control using a **classical FNO** (real DFT/IDFT only),
same spectral construction as `burgers_bc_data/train_fno_burgers_bc_real_freq.py`.

Differs from `train_ddpg_heat_efno.py`, which uses `EFNO2d` (learned complex inverse frequencies).

Run from repo root:
  python -m heat_rl_efno.train_ddpg_heat_fno_real_freq
"""
...
if __package__ in (None, ""):
    ...
    from heat_rl_efno.models import gather_action_time, state_tensor
    from heat_rl_efno.models_fno_real import ActorFNORealFreq, CriticQFNORealFreq
    from heat_rl_efno.replay_buffer import ReplayBuffer
    from heat_rl_efno.rollouts import rollout_policy
    from heat_rl_efno.train_args import add_shared_args
else:
    from .heat_env import HeatEnv, HeatEnvConfig, heat_env_defaults
    from .models import gather_action_time, state_tensor
    from .models_fno_real import ActorFNORealFreq, CriticQFNORealFreq
    from .replay_buffer import ReplayBuffer
    from .rollouts import rollout_policy
    from .train_args import add_shared_args
...
def train(args):
    ...
    print("Backbone: FNO (real frequency), cf. train_fno_burgers_bc_real_freq.py")
    ...
    actor = ActorFNORealFreq(
        nx, nt, k_max=args.k_max, width=args.width, n_layers=args.n_layers, a_max=args.a_max
    ).to(device)
    actor_tgt = ActorFNORealFreq(
        nx, nt, k_max=args.k_max, width=args.width, n_layers=args.n_layers, a_max=args.a_max
    ).to(device)
    actor_tgt.load_state_dict(actor.state_dict())

    critic = CriticQFNORealFreq(nx, nt, k_max=args.k_max, width=args.width, n_layers=args.n_layers).to(device)
    critic_tgt = CriticQFNORealFreq(nx, nt, k_max=args.k_max, width=args.width, n_layers=args.n_layers).to(device)
    ...
    for ep in range(1, args.episodes + 1):
        ...
        #         "cfg": cfg,
        #         "args": {**vars(args), "rl_backbone": "fno_real_freq"},
        #     }
...
def main():
    ...
    add_shared_args(p, env_d)
    p.add_argument("--out-dir", type=str, default="heat_rl_fno_real_freq_output")
    args = p.parse_args()
    train(args)
